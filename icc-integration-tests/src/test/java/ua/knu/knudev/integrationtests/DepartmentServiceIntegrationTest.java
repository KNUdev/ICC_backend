...
public class DepartmentServiceIntegrationTest {
    ...
    class GetByIdScenarios {
        @Test
        @Transactional
        @DisplayName("Should successfully get department when provided valid id")
        public void should_SuccessfullyGetDepartment_When_ProvidedValidId() {
            DepartmentDto response = departmentService.getById(testDepartment.getId());
            ...
            assertEquals(testDepartment.getCreatedAt(), response.createdAt());
            assertEquals(testDepartment.getUpdatedAt(), response.updatedAt());
            assertEquals(1, testDepartment.getApplications().size());
        }

        @Test
        @Transactional
        @DisplayName("Should throw DepartmentException when provided invalid id")
        public void should_ThrowDepartmentException_When_ProvidedInvalidId() {
            assertThrows(DepartmentException.class, () -> departmentService.getById(UUID.randomUUID()));
        }
    }
    ...
    @Test
    @Transactional
    @DisplayName("Should successfully update department when provided valid data")
    public void should_SuccessfullyUpdateDepartment_When_ProvidedValidDataInRequest() {
        DepartmentUpdateRequest request = new DepartmentUpdateRequest(testDepartment.getId(),
                new MultiLanguageFieldDto("Updated-department-name", "Оновлена-назва-факультету"));

        DepartmentDto response = departmentService.update(request);
        assertNotNull(response);
        assertEquals("Updated-department-name", response.name().getEn());
        assertEquals("Оновлена-назва-факультету", response.name().getUk());
        assertTrue(departmentRepository.existsById(response.id()));
    }
    ...
    class GetAllScenarios {
        @Test
        @Transactional
        @DisplayName("Should successfully get all department when provided appropriate request")
        public void should_SuccessfullyGetAllDepartment_When_ProvidedAppropriateRequest() {
            for (int i = 0; i < 9; i++) {
                createTestDepartment();
            }

            DepartmentGetAllRequest request = new DepartmentGetAllRequest(
                    TEST_DEPARTMENT_NAME_IN_ENGLISH,
                    TEST_APPLICATION_PROBLEM_DESCRIPTION,
                    null,
                    null,
                    null,
                    null,
            ...
            Page<DepartmentDto> response = departmentService.getAll(request);
        ...
        @Test
        @Transactional
        @DisplayName("Should get zero departments when provided not existing search query")
        public void should_SuccessfullyGetAllDepartment_When_ProvidedNotExistingSearchQuery() {
            for (int i = 0; i < 9; i++) {
                createTestDepartment();
            }
